...
/**
 * Cliente S3 Ligero V4 compatible con Wasabi, Contabo, AWS, Cloudflare y Backblaze.
 */
...
class StorageManager {
    ...
    public function __construct($userId = null) {
        ...
        // CORRECCIÓN: Se agrega backblaze y cloudflare a la inicialización
        if ($this->activeConfig && in_array($this->activeConfig['storage_type'], ['contabo', 'wasabi', 'aws', 'backblaze', 'cloudflare'])) {
    ...
    public function uploadFile($file, $targetName) {
        ...
        $tmp = $file['tmp_name'];
        // CORRECCIÓN: Se agrega backblaze y cloudflare al array de subida
        if (in_array($this->activeConfig['storage_type'], ['contabo','wasabi','aws','backblaze','cloudflare'])) {
    ...
    public function deleteFile($path, $type = null) {
        ...
        // CORRECCIÓN: Se agrega backblaze y cloudflare al array de borrado
        if (in_array($config['storage_type'], ['contabo','wasabi','aws','backblaze','cloudflare'])) {

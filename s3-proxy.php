<?php
// s3-proxy.php - Proxy Inteligente con Retrocompatibilidad y Seguridad
require_once 'config/app.php';
require_once 'includes/functions.php';
require_once 'includes/db_connect.php';

// --- 1. SEGURIDAD: CONTROL DE DOMINIOS (Hotlink Protection) ---
if (defined('ALLOWED_DOMAINS') && is_array(ALLOWED_DOMAINS) && !empty(ALLOWED_DOMAINS)) {
    ...
    $isAllowed = ($refererHost === $selfHost); // Permitir propio sitio
    
    if (!$isAllowed) {
        foreach (ALLOWED_DOMAINS as $d) {
            if (strtolower(trim($d)) === strtolower($refererHost)) {
...
class S3ProxyV4 {
    ...
    public function stream($path) {
        ...
        if (!$res || $code >= 400) {
            if(!headers_sent()) { http_response_code($code == 404 ? 404 : 500); echo "Error streaming ($code)"; }
            exit;
        }
    }
...
// === MAIN ===
$v = $_GET['v'] ?? '';
if (!$v) { header('HTTP/1.1 400 Bad Request'); exit; }
...
if (!$video || $video['status'] !== 'active') { header('HTTP/1.1 404 Not Found'); exit; }

// 2. Obtener Credenciales (Retrocompatibilidad)
// BUSCAMOS POR TIPO DE ALMACENAMIENTO, NO IMPORTA SI ES ACTIVO O NO
$storage = null;
